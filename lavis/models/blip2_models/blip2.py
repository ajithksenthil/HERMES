    ...
    def forward(
        ...
        if is_cross_attention:
            ...
            if hasattr(self, "query_memory_bank"):
                ...
                query_memory_bank = self.query_memory_bank.view(
                    B, -1, C
                )  # [B, T*32, C]
                query_memory_bank_k = torch.cat(
                    [self.key(query_memory_bank), k], dim=1
                )  # [B, (T+1)*32, C]
                query_memory_bank_v = torch.cat(
                    [self.value(query_memory_bank), v], dim=1
                )  # [B, (T+1)*32, C]
        ...
        if not is_cross_attention:
            ...
            if not hasattr(self, "query_memory_bank"):
                self.query_memory_bank = hidden_states[
                    :, None, :, :
                ].detach()  # [B, 1, 32, C]
                self.size_constant = torch.ones(B, 1, N).to(
                    hidden_states.device
                )  # [B, 1, N]
                self.compression_size = self.size_constant
            else:
                self.query_memory_bank = torch.cat(
                    [self.query_memory_bank, hidden_states[:, None, :, :].detach()],
                    dim=1,
                )  # [B, t+1, 32, C]
            ...
            if self.compression_size.sum(1).mean().round() == self.num_frames:
                ...
                self.query_memory_bank, self.compression_size = episodic_compressor(
...
def episodic_compressor(
    memory_bank: torch.Tensor, compression_size: torch.Tensor, return_size: int = 5
) -> tuple:
    """
    Compresses the memory bank tensor by iteratively merging the most similar adjacent frames until the desired return size is achieved.

    This function implements the Episodic Compression Operation (ECO) from the referenced paper. It calculates the cosine similarity between adjacent frames in the memory bank and merges the frames with the highest similarity. The merging process involves summing the memory banks and their corresponding sizes, followed by normalization.

    Args:
        memory_bank (torch.Tensor): A tensor of shape (B, T, N, C) representing the memory bank, where B is the batch size, T is the number of frames, N is the number of tokens, and C is the number of channels.
        compression_size (torch.Tensor): A tensor of shape (B, T, N) representing the size of each memory bank entry.
        return_size (int, optional): The desired number of frames to retain in the memory bank after compression. Default is 5.

    Returns:
        tuple: A tuple containing the compressed memory bank tensor and the corresponding compression sizes. The shapes of the returned tensors are (B, return_size, N, C) and (B, return_size, N), respectively.

    Raises:
        AssertionError: If return_size is greater than the number of frames (T) in the memory bank.
    """
    B, T, N, C = memory_bank.shape

    assert (
        return_size <= T
    ), "retain_frames must be less than or equal to the number of frames in the memory bank."

    if T <= return_size:
        return memory_bank, compression_size
    ...
    while T > return_size:
        with torch.no_grad():
            ...
            _, max_similarity_indices = torch.max(
            ...
            dst_indices = (
                torch.arange(T - 1)
                .to(memory_bank.device)[None, :, None]
                .repeat(B, 1, N)
            )
            ...
            memory_bank = dst_memory_bank / dst_size.unsqueeze(-1)
            compression_size = dst_size
            T = memory_bank.shape[1]
...
def reset_memory_banks(model):
    for module in model.modules():
        if isinstance(module, MBBertSelfAttention):
            module.reset_memory_bank()
    return model
