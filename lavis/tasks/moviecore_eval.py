...
from lavis.tasks.moviecore_eval_prompts import get_prompt
    ...
    for evaluation_dimension in evaluation_dimensions:
        ...
        for attempt in range(retries):
            try:
                ...
                response_message = (
                    response_message.replace("```python", "").replace("```", "").strip()
                )
...
def calculate_scores(results_list):
    ...
    final_results["overall"] = {
        dim: (
            overall_scores[dim]["total"] / overall_scores[dim]["count"]
            if overall_scores[dim]["count"] > 0
            else 0
        )
    ...
    for classification in classification_scores:
        ...
        dimension_scores = {
            dim: (
                classification_scores[classification][dim]["total"]
                / classification_scores[classification][dim]["count"]
                if classification_scores[classification][dim]["count"] > 0
                else 0
            )
            for dim in evaluation_dimensions
        }
        ...
        final_results["by_classification"][classification][
            "average_across_dimensions"
        ] = avg_across_dimensions
